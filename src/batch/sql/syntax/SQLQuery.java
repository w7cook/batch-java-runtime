...
public class SQLQuery extends Base {
  ...
  // used for root query
  public SQLQuery(ISchema schema, Loop loop, SQLAction sqlAction,
      boolean isValue, SQLQuery parentQuery) {
    this.schema = schema;
    this.loop = loop;
  ...
  public void addProjection(Output n) {
    ...
    if (getAggOp() != null)
      ...
      if (groupBy != null && n.getExpr() instanceof SQLQuery) {
        ...
        n = new Output(n.getLocation(), new Call(
            batch.sql.syntax.Factory.factory.Root(), inner.getAggOp(), args));
      }
      bindings.add(n);
    }
  }
  ...
  public void addSubquery(SQLQuery subquery, SQLTranslation parentBase) {
    ...
    if (outputKey == null && parentBase != null) {
      SQLTranslation exp = parentBase.normalize(schema, this, null, env,
          NormType.NORMAL);
  ...
  public void addWhere(SQLTranslation cond) {
    where = batch.syntax.Parser.binary(Factory.factory, Op.AND, cond, where);
  }
  ...
  public void toSQL(StringBuilder sb, List<Object> params, ForestReader data) {
    ...
    case SELECT:
      ...
      if (aggregate && op != Op.OR) {
        ...
        sb.append(')');
      } else if (isValue && body != null && bindings.size() == 0) {
        if (body instanceof SQLTable)
          body = ((SQLTable) body).getKey();
        body.toSQL(sb, params, data);
      } else {
  ...
  private void outputFrom(StringBuilder sb, List<Object> params,
      ForestReader data) {
    if (tables != null) {
      sb.append("\nFROM ");
      tables.fromClause(sb);
  ...
  private void outputWhere(StringBuilder sb, List<Object> params,
      ForestReader data) {
    if (where != null) {
      sb.append("\nWHERE ");
  ...
  public SQLTranslation finalize(SQLTranslation body, SQLTranslation testId) {
    ...
    if (testId != null) {
      SQLTranslation id = testId.normalize(schema, this, null, env,
          NormType.NORMAL);
      addWhere(Factory.factory.Prim(Op.EQ, env.lookup(getVar()).getTable()
          .getKey(), id));
    }
