...
public class If extends Base {
  ...
  public SQLTranslation normalize(ISchema schema, SQLQuery query,
    ...
    SQLTranslation newcond = cond.normalize(schema, query, null, env, normType);
    ...
    if (!(cond instanceof Output)) {
      ...
      if (condTable == null) { // must be a boolean!
        boolean test = ((Boolean) newcond.run(new Eval()).evaluate(null, null,
            null)).booleanValue();
        if (test)
    ...
    if (optCond) {
      thenExp.normalize(schema, query,
          batch.syntax.Parser.binary(Factory.factory, Op.AND, outerCond, cond),
          env, normType);
      SQLTranslation notCond = Factory.factory.Prim(Op.NOT, cond);
      elseExp.normalize(schema, query, batch.syntax.Parser.binary(
          Factory.factory, Op.AND, outerCond, notCond), env, normType);
      return Factory.factory.Skip();
    } else {
      SQLTranslation newthenExp = thenExp.normalize(schema, query, outerCond,
          env, normType);
      SQLTranslation newelseExp = elseExp.normalize(schema, query, outerCond,
          env, normType);
