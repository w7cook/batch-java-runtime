...
public class Factory extends BatchFactoryHelper<SQLTranslation> {
  ...
  public static boolean isRoot(SQLTranslation target) {
    return target instanceof Var && ((Var)target).name.equals(batch.sql.syntax.Factory.factory.RootName());
  }

	@Override
	public SQLTranslation Assign(Op op, SQLTranslation target, SQLTranslation value) {
		return new Assign(op, target, value);
	}
	...
	@Override
	public SQLTranslation If(SQLTranslation cond, SQLTranslation thenExp, SQLTranslation elseExp) {
	...
	@Override
	public SQLTranslation Call(SQLTranslation target,
			String method,
		...
		if (method.startsWith("set") && args.size() == 1)
			return new Assign(Op.SEQ,
					new Dot(target, method.substring(3)),
					args.get(0));
		// aggregations
	...
	private Loop makeAggregation(SQLTranslation collection,
			Op op,
			List<SQLTranslation> args) {
	...
	private Loop makeWhere(SQLTranslation collection, List<SQLTranslation> args) {
		...
		return new Loop(var, collection, new If(fun.getBody(),
				new Var(var),
				factory.Skip()));
	}
	...
	@Override
	public SQLTranslation Loop(String var,
